pipeline {
    ...
    environment {
        DOCKERHUB_CREDENTIALS=credentials('dockerhub_forjen')
        NAME = 'thaidam'
        HOVATEN = 'DAMTHAI'
        abc = 'asdf'
    }
    stages {
        ...
        stage('Package to docker image') {
            ...
            steps {
                ...
                sh 'ls -la'
                sh 'ls -la target'
                sh 'docker build -t thaidam/myrepo:1.0 .'
            }
        }
        stage('Pushing image') {
            steps {
                ...
                sh 'docker push thaidam/myrepo:1.0'
        ...
        stage('Deploying and Cleaning') {
            steps {
                ...
                sh 'docker image rm thaidam/myrepo:1.0 || echo "this image does not exist" '
                ...
                sh 'docker container run -d --rm --name my-demo-springboot -p 8082:8080 --network jenkins thaidam/myrepo:1.0'
